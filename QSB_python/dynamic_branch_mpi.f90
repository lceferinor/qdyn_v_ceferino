...
module okada
  ...
  public :: compute_kernel_gen, UA0, UB0, UC0, DC3, UA, UB, UC, DCCON0, DCCON1, DCCON2
...
!
subroutine compute_kernel_gen(LAM,MU,SX,SY,SZ,S_DIP,L,W,OX,OY,OZ,O_DIP,Ustrike,Udip,mode,tau,sigma,IRET)

  integer :: mode
!f2py intent(in) :: mode
  double precision :: LAM,MU,SX,SY,SZ,S_DIP,L,W,OX,OY,OZ,O_DIP,Ustrike,Udip
!f2py intent(in) :: LAM,MU,SX,SY,SZ,S_DIP,L,W,OX,OY,OZ,O_DIP,Ustrike,Udip 
  integer :: IRET
!f2py intent(out) :: IRET
  double precision :: tau, sigma
!f2py intent(out) :: tau, sigma

  double precision, parameter :: PI = 3.141592653589793d0
  double precision :: ALPHA, S_DEPTH, X, Y, Z, &
    UX,UY,UZ,UXX,UYX,UZX,UXY,UYY,UZY,UXZ,UYZ,UZZ,Unorm
...
!Normal vector of receiver fault
  ...
  call DC3(ALPHA,X,Y,Z,S_DEPTH,S_DIP,-0.5d0*L,0.5d0*L,-0.5d0*W,0.5d0*W, &
            Ustrike,Udip,Unorm,   &
...
end subroutine compute_kernel_gen
...
!=========================================                              
  ...
  555   U(I)=U(I)+POT4/PI2*DU(I)                                        
      ...
      SUBROUTINE  DC3(ALPHA,X,Y,Z,DEPTH,DIP,    &                       
      ...
      DOUBLE PRECISION :: ALP1,ALP2,ALP3,ALP4,ALP5,SD,CD,SDSD,CDCD,SDCD,S2D,C2D
...
!-----                                                                  
      ...
      IF(Z.GT.0.) WRITE(6,'('' ** POSITIVE Z WAS GIVEN IN SUB-DC3'')') 
...
integer :: N_main,N_branch,mode
integer :: i,j,IRET
double precision :: lam,mu,DIPM,DLM,DWM,DIP0
double precision :: itau_new,isigma_new,isigma,itau
!For main fault
double precision, dimension(:),allocatable :: X_m,Y_m,Z_m
!For Branch fault
!double precision, dimension(:), allocatable :: X_b,Y_b,Z_b,sigma_new,tau_new,V_new,th_new
!double precision, dimension(:), allocatable :: SIGMA_b,TAU_b,V0_b,A_b,B_b,DC_b,MUSS_b,C0_b
real :: Udip
double precision, dimension(:), allocatable :: X_b,Y_b,Z_b,sigma_new,tau_new,V_new,th_new
double precision, dimension(:), allocatable :: SIGMA_b,TAU_b,V0_b,A_b,B_b,DC_b,MUSS_b,C0_b,Ustrike
   
!for MPI 
integer :: ier, MY_RANK=0,NPROCS=1,iproc,N_branch_all,ierr
character(len=6) :: iprocnum

double precision, dimension(:), allocatable :: X_all,Y_all,Z_all,sigma_new_all,tau_new_all,V_new_all,th_new_all
double precision, dimension(:), allocatable :: V0_all,A_all,B_all,DC_all,MUSS_all,C0_all
integer, dimension(:), allocatable :: nN_branch_perproc,noffN_branch_all
double precision :: phi,xi,yi
double precision :: pi
!!!
...
write(iprocnum,'(i1)') MY_RANK
write(6,*) 'dynamic_branch'//iprocnum(1:len_trim(iprocnum))//'.par'
open(unit=10,FILE='dynamic_branch'//iprocnum(1:len_trim(iprocnum))//'.par',ACTION='read',STATUS='UNKNOWN',iostat=ierr)
if (ierr/=0) stop 'Error during reading a file'
read(10,*) N_main
allocate(X_m(N_main),Y_m(N_main),Z_m(N_main))
read(10,*) lam,mu,DIPM,DLM,DWM
do i=1,N_main
  read(10,*) X_m(i),Y_m(i),Z_m(i)
enddo
read(10,*) N_branch
! Synchronize all
allocate(nN_branch_perproc(0:NPROCS-1))
allocate(noffN_branch_all(0:NPROCS-1))

!  call gather_alli(N_branch,nN_branch_perproc,NPROCS)
  call MPI_ALLGATHER(N_branch,1,MPI_INTEGER, &
                  nN_branch_perproc,1,MPI_INTEGER, &
                  MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering integer MPI processes'
write(6,*) 'MY_RANK:',MY_RANK,'nNbranch_perproc:',nN_branch_perproc
do iproc=0,NPROCS-1
   noffN_branch_all(iproc)=sum(nN_branch_perproc(0:iproc))-nN_branch_perproc(iproc)
enddo

N_branch_all = sum(nN_branch_perproc(0:NPROCS-1))
write(6,*) N_branch_all,'MY_RANK:',MY_RANK 

! Allocating global arrays 
allocate(X_all(N_branch_all),Y_all(N_branch_all),Z_all(N_branch_all))
allocate(V0_all(N_branch_all),A_all(N_branch_all),B_all(N_branch_all),DC_all(N_branch_all),&
         MUSS_all(N_branch_all),C0_all(N_branch_all))
allocate(sigma_new_all(N_branch_all),tau_new_all(N_branch_all),V_new_all(N_branch_all),th_new_all(N_branch_all))

! Allocating local arrays 
allocate(X_b(N_branch),Y_b(N_branch),Z_b(N_branch))
allocate(SIGMA_b(N_branch),TAU_b(N_branch),V0_b(N_branch),A_b(N_branch),B_b(N_branch),DC_b(N_branch),&
         MUSS_b(N_branch),Ustrike(N_branch),C0_b(N_branch))
allocate(sigma_new(N_branch),tau_new(N_branch),V_new(N_branch),th_new(N_branch))

! Reading local variables
read(10,*) mode
read(10,*) DIP0
do i=1,N_branch
  read(10,"(E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6,1x,E20.6)") &
             X_b(i),Y_b(i),Z_b(i),SIGMA_b(i),TAU_b(i),V0_b(i),&
             A_b(i),B_b(i),DC_b(i),MUSS_b(i),Ustrike(i),Udip,C0_b(i)
enddo
close(10)
...
do j=1,N_branch
   itau_new = 0.0d0
   isigma_new = 0.0d0
   if (MY_RANK==0) write(6,*) j
   do i=1,N_main
     call compute_kernel_gen(lam,mu,X_m(i),Y_m(i),Z_m(i),DIPM,DLM,DWM,&
          dble(X_b(j)),dble(Y_b(j)),dble(Z_b(j)),DIP0,dble(Ustrike(j)),dble(Udip),mode,itau,isigma,IRET)
     itau_new = itau + itau_new
     isigma_new = isigma + isigma_new
   enddo
   sigma_new(j) = SIGMA_b(j) + isigma_new
   tau_new(j)   = TAU_b(j) + itau_new
   if (abs(A_b(j)-B_b(j))<0.25*B_b(j)) then 
      V_new(j) = V0_b(j)
   else
      V_new(j) = V0_b(j)*exp((tau_new(j)/sigma_new(j)-MUSS_b(j))/(A_b(j)-B_b(j)))
   endif
   th_new(j) = DC_b(j)/V_new(j)
   if (MY_RANK==NPROCS-1) write(6,*) 'j:',j
enddo

  call MPI_GATHERV(X_b,N_branch,MPI_DOUBLE_PRECISION,X_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(Y_b,N_branch,MPI_DOUBLE_PRECISION,Y_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(Z_b,N_branch,MPI_DOUBLE_PRECISION,Z_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(sigma_new,N_branch,MPI_DOUBLE_PRECISION,sigma_new_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(tau_new,N_branch,MPI_DOUBLE_PRECISION,tau_new_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(V0_b,N_branch,MPI_DOUBLE_PRECISION,V0_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(MUSS_b,N_branch,MPI_DOUBLE_PRECISION,MUSS_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(A_b,N_branch,MPI_DOUBLE_PRECISION,A_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(B_b,N_branch,MPI_DOUBLE_PRECISION,B_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(DC_b,N_branch,MPI_DOUBLE_PRECISION,DC_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(V_new,N_branch,MPI_DOUBLE_PRECISION,V_new_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(th_new,N_branch,MPI_DOUBLE_PRECISION,th_new_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'

  call MPI_GATHERV(C0_b,N_branch,MPI_DOUBLE_PRECISION,C0_all,nN_branch_perproc,&
                      noffN_branch_all,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,ier)
if (ier /= 0 ) stop 'Error gathering real MPI processes'
...
! Rotation
  do i =1,N_branch_all
   xi=X_all(i)
   yi=Y_all(i)
   X_all(i)=  xi*cos(phi*pi/180.) + yi*sin(phi*pi/180.)
   Y_all(i)= -xi*sin(phi*pi/180.) + yi*cos(phi*pi/180.)
  enddo

  write(6,*) 'mintau',minval(tau_new_all)
  write(6,*) 'maxtau',maxval(tau_new_all)
  write(6,*) 'Start write SEM branch input: rsf_hete_input_file_fault2.txt'
  open(unit=11,FILE='rsf_hete_input_file_fault2.txt',ACTION='write',STATUS='UNKNOWN')
  write(11,*) N_branch_all
  do i=1,N_branch_all
    write(11,"(14E24.6)") X_all(i),Y_all(i),Z_all(i),sigma_new_all(i),tau_new_all(i),Udip,V0_all(i),&
                                                    MUSS_all(i),A_all(i),B_all(i),DC_all(i),V_new_all(i),th_new_all(i),C0_all(i)
  enddo
  close(11)
endif
