...
%pars
for istart=1:1:1
mname='nodesonfault.csv'; %SEM mesh
...
model='/Users/percy.galvez/Dropbox/qdyn_developer/examples/SMGA_cycle/Mw6_7/DC_mean_0.025_dc_sigma_0.25_col_3km/'
filename = [model,mname]
%load mesh
...
%q.NZ=0;
%while q.Z(q.NZ+1) == q.Z(1)
%    q.NZ=q.NZ+1;
%end
idz=find(abs(q.Z-q.Z(1))<1);
q.NX = length(idz);
idx=find(sqrt((q.X-min(q.X)).^2)<1);
q.NZ = length(idx);
