...
# Rotation
for i in range(qN):
   qX[i] =  o.X[i]*np.cos(phi*np.pi/180.0)+o.Y[i]*np.sin(phi*np.pi/180.0)
   qY[i] = -o.X[i]*np.sin(phi*np.pi/180.0)+o.Y[i]*np.cos(phi*np.pi/180.0)
...
for i in range(qN):
#    idexmin=np.argmin(np.square(p.X-qX[i])+np.square(p.Y-qY[i])+np.square(p.Z-qZ[i])) 
#    idex=idexmin
    idex=i
...
fid=open('rsf_hete_input_file_fault1.txt','w')
...
for i in range(qN):
    txt='%E %E %E %E %E %E %E %E %E %E %E %E %E %E\n'%(qX[i],qY[i],qZ[i],qSIGMA[i],tau[i],0.0,qV0[i],qf0[i],qA[i],qB[i],qDC[i],qV_ini[i],qTH_ini[i],co[i])
    fid.write(txt)
    if (i%math.ceil(qN/100)==0):
        print(i/math.ceil(qN/100), '% Complete')

print 'Generated SEM input: rsf_hete_input_file1.txt'
fid.close()
